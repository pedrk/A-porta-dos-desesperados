<template>
    <div class="door-area">
        <div class="door-frame" :class="{ selected: selected && !open }">
            <!-- A porta vai estar com o presente se a porta tiver aberta e for a escolhida -->
            <Gift v-if="open && hasGift" />
        </div>
        <div class="door" :class="{ open }"
            @click="selected = !selected">
            <div class="number" :class="{ selected }">{{ number }}</div>
            <div class="macaneta" :class="{ selected }"
...
export default {
    name: 'Door', // Nome do componente
    components: { Gift }, // Componentes importados
    props: {// Propriedades que não serão mudadas 
        number: {}, 
        hasGift: { type: Boolean } 
    },
    data: function () {
        return {// Dados que podem mudar
            open: false,
            selected: false
        }
    }
